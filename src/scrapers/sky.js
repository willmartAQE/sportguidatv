...
function cleanTitle(value) {
  return value.replace(/\s+/g, ' ').replace(/\bIN ONDA\b/gi, '').replace(/\b(?:Sport|Rubrica|Film|Serie TV|Documentari)\s*\(\d+['’]?\)\b/gi, '').trim();
}
...
function parseChannelPage(channel, html) {
  ...
  for (const node of $('body *').toArray()) {
    ...
    if (!match || text.length > 500 || $(node).find('*').length === 0) continue;
    const startTime = match[0].replace('.', ':').padStart(5, '0');
    const afterTime = text.slice(match.index + match[0].length);
    const title = cleanTitle(afterTime).split(/\s{2,}/)[0];
    if (!title || title.length < 3 || /^\d+$/.test(title)) continue;

...
export default async () => {
  ...
  const results = await Promise.all(sources.map(async ([name, fetcher]) => {
    ...
    try {
      ...
      console.log('Scraper result', JSON.stringify({ name, ok: true, count: events.length, durationMs: Date.now() - startedAt }));
      return { name, ok: true, count: events.length, error: result?.error || null, events };
    } catch (error) {
      console.error('Scraper failed', JSON.stringify({ name, message: error?.message || String(error), durationMs: Date.now() - startedAt }));

...
export const handler = async (event) => {
  ...
  try {
    ...
    bot.action(/^sport:(?!calcio)(.+)$/, async (ctx) => { await ctx.answerCbQuery(); const sport = ctx.match[1]; const cache = await getCache(); const today = new Date().toISOString().slice(0, 10); const events = filterEvents(cache.events, sport, null, today); await ctx.editMessageText(formatSchedule(sport, 'today', events), { parse_mode: 'HTML', reply_markup: backKeyboard() }); });
    bot.action(/^football:(serie-a|serie-b|serie-c)$/, async (ctx) => { await ctx.answerCbQuery(); const competition = ctx.match[1]; const cache = await getCache(); const today = new Date().toISOString().slice(0, 10); const events = filterEvents(cache.events, 'calcio', competition, today); const label = competition.replace('serie-', 'Serie ').toUpperCase(); await ctx.editMessageText(formatSchedule('Calcio', 'today', events, label), { parse_mode: 'HTML', reply_markup: backKeyboard() }); });

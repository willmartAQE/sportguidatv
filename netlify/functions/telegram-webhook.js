...
export const handler = async (event) => {
  ...
  try {
    ...
    bot.action(/^menu:sports(?::(today|tomorrow))?$/, async (ctx) => { await ctx.answerCbQuery(); const day = ctx.match[1] || 'today'; await ctx.editMessageText(day === 'tomorrow' ? '📅 <b>Domani</b>\n\nSeleziona uno sport:' : menuText(), { parse_mode: 'HTML', reply_markup: sportKeyboard(day) }); });
    bot.action(/^sport:(?!calcio)([a-z-]+):(today|tomorrow)$/, async (ctx) => { await ctx.answerCbQuery(); const sport = ctx.match[1]; const day = ctx.match[2]; const offset = day === 'tomorrow' ? 1 : 0; const cache = await getCache(); const events = filterEvents(cache.events, sport, null, italyDate(offset)); await ctx.editMessageText(formatSchedule(sportLabels[sport] || sport, day, events), { parse_mode: 'HTML', reply_markup: backKeyboard(day) }); });
    bot.action(/^sport:calcio:(today|tomorrow)$/, async (ctx) => { await ctx.answerCbQuery(); const day = ctx.match[1]; await ctx.editMessageText(footballText(), { parse_mode: 'HTML', reply_markup: footballKeyboard(day) }); });
    bot.action(/^football:(serie-a|serie-b|serie-c):(today|tomorrow)$/, async (ctx) => { await ctx.answerCbQuery(); const competition = ctx.match[1]; const day = ctx.match[2]; const offset = day === 'tomorrow' ? 1 : 0; const cache = await getCache(); const events = filterEvents(cache.events, 'calcio', competition, italyDate(offset)); const label = competition.replace('serie-', 'Serie ').toUpperCase(); await ctx.editMessageText(formatSchedule('Calcio', day, events, label), { parse_mode: 'HTML', reply_markup: backKeyboard(day) }); });
    bot.action('day:today', async (ctx) => { await ctx.answerCbQuery('Aggiornato'); await ctx.editMessageText(menuText(), { parse_mode: 'HTML', reply_markup: sportKeyboard('today') }); });
